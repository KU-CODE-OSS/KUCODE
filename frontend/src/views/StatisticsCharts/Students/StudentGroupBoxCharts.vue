  ...
  methods: {
    updateChartData(data) {
      ...
      data.forEach(item => {
        ...
        yearGroups[year][item[this.groupKey]] = [stats.min, stats.q1, stats.median, stats.q3, stats.max];
      });
      ...
      for (const year in yearGroups) {
        ...
        courses.forEach(course => {
          ...
          if (courseData.length !== 5) {
            console.error(`데이터 문제 발견: ${year} - ${course}: 5개의 요소가 필요하지만 ${courseData.length}개가 발견됨`);
          }
          row.push(...courseData);
        });
        // 행의 열 수가 올바른지 확인
