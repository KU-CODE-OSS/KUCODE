<template>
  <div>
    ...
    <!-- <v-card> -->
      <v-tabs align="right" v-model="table_or_chart">
        ...
        <!-- ↓↓↓ 차트 컴포넌트 ↓↓↓ -->
            <v-lazy>
                ...
                <v-row>
                  ...
                  <v-col cols="6">
                    <chart_user_example
                      :averageCommitsByCourse="averageCommitsByCourse" 
                      :averagePRByCourse="averagePRByCourse"
                      :averageIssueByCourse="averageIssueByCourse"
                      :averageRepoByCourse="averageRepoByCourse"
                    />
                  </v-col>
                </v-row>
...
export default {
  ...
  data() {
    return {
      ...
      totalIssueByCourse: {},
      totalRepoByCourse: {},
      courseCounts: {}, // 과목별 학생 수를 저장하기 위한 변수
      averageCommitsByCourse: {}, // 과목별 평균 commit을 저장하기 위한 변수
      averagePRByCourse: {}, // 과목별 평균 PR을 저장하기 위한 변수
      averageIssueByCourse: {}, // 과목별 평균 Issue를 저장하기 위한 변수
      averageRepoByCourse: {} // 과목별 평균 Repo를 저장하기 위한 변수
    };
  },
  created() {
    this.fetchTotal();
    ...
    processCourse(data) {
      ...
      data.forEach(student => {
        ...
        if (!totalCommitsByCourse[student.course_name]) {
          ...
          courseCounts[student.course_name] = 0;
        }
        ...
        // course_name별 학생 수 계산
        courseCounts[student.course_name] += 1;

        console.log(`이름: ${student.name} Course: ${student.course_name}, Commit: ${student.commit}, PR: ${student.pr}, Issue: ${student.issue}, Repo: ${student.num_repos}`);
      ...
      // 과목별 평균 계산
      const averageCommitsByCourse = {};
      const averagePRByCourse = {};
      const averageIssueByCourse = {};
      const averageRepoByCourse = {};

      Object.keys(totalCommitsByCourse).forEach(course => {
        averageCommitsByCourse[course] = (totalCommitsByCourse[course] / courseCounts[course]).toFixed(3);
        averagePRByCourse[course] = (totalPRByCourse[course] / courseCounts[course]).toFixed(3);
        averageIssueByCourse[course] = (totalIssueByCourse[course] / courseCounts[course]).toFixed(3);
        averageRepoByCourse[course] = (totalRepoByCourse[course] / courseCounts[course]).toFixed(3);
      });

      console.log('Average Commits by Course:', averageCommitsByCourse);
      console.log('Average PR by Course:', averagePRByCourse);
      console.log('Average Issue by Course:', averageIssueByCourse);
      console.log('Average Repo by Course:', averageRepoByCourse);
      ...
      this.averageCommitsByCourse = averageCommitsByCourse; // 저장
      this.averagePRByCourse = averagePRByCourse; // 저장
      this.averageIssueByCourse = averageIssueByCourse; // 저장
      this.averageRepoByCourse = averageRepoByCourse; // 저장
    },
