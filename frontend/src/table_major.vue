          ...
          <v-container grid-list-md>
            <v-row wrap>
              <v-col xs="12" sm="12" md="6">
                <v-text-field
                  v-model="importItem.course_id"
                  label="학수번호"
                  :rules="[() => !!importItem.course_id || '필수 입력 항목입니다']"
                  @input="validateForm"
                ></v-text-field>
              </v-col>
              <v-col xs="12" sm="12" md="6">
                <v-text-field
                  v-model="importItem.year"
                  label="연도"
                  :rules="[
                    () => !!importItem.year || '필수 입력 항목입니다',
                    () => /^\d+$/.test(importItem.year) || '올바른 필드 값이 아닙니다'
                  ]"
                  @input="validateForm"
                ></v-text-field>
              </v-col>
              <v-col xs="12" sm="12" md="6">
                <v-text-field
                  v-model="importItem.semester"
                  label="학기"
                  :rules="[
                    () => !!importItem.semester || '필수 입력 항목입니다',
                    () => /^\d+$/.test(importItem.semester) || '올바른 필드 값이 아닙니다'
                  ]"
                  @input="validateForm"
                ></v-text-field>
              </v-col>
              <v-col xs="12" sm="12" md="6">
                <v-text-field
                  v-model="importItem.name"
                  label="과목명"
                  :rules="[() => !!importItem.name || '필수 입력 항목입니다']"
                  @input="validateForm"
                ></v-text-field>
              </v-col>
              <v-col xs="12" sm="12" md="6">
                <v-text-field
                  v-model="importItem.prof"
                  label="교수"
                  :rules="[() => !!importItem.prof || '필수 입력 항목입니다']"
                  @input="validateForm"
                ></v-text-field>
              </v-col>
              <v-col xs="12" sm="12" md="6">
                <v-text-field
                  v-model="importItem.ta"
                  label="조교"
                  :rules="[() => !!importItem.ta || '필수 입력 항목입니다']"
                  @input="validateForm"
                ></v-text-field>
              </v-col>
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        ...
        validations: {
          ...
          ta: false,
          student_count: false,
          course_repo_name: false,
        },
      innerButtonText: 'Import',
    };
    },
    computed: {
      formTitle() {
    ...
    openImportDialog() {
      ...
      this.innerButtonText = '확인';
    },
      validateForm() {
      this.validations.course_id = !!this.importItem.course_id;
      this.validations.year = !!this.importItem.year && /^\d+$/.test(this.importItem.year);
      this.validations.semester = !!this.importItem.semester && /^\d+$/.test(this.importItem.semester);
      this.validations.name = !!this.importItem.name;
      this.validations.prof = !!this.importItem.prof;
      this.validations.ta = !!this.importItem.ta;
      },
      selectFile(event) {
      ...
      this.importItem.course_repo_name = `${this.importItem.year}${this.importItem.semester}${this.importItem.course_id}`;
      ...
      const reader = new FileReader();
        ...
        XLSX.utils.book_append_sheet(newWorkbook, newWorksheet, 'Sheet1');
          ...
          const newFile = new Blob([newExcelBuffer], {
        ...
        formData.append('course_repo_name', this.importItem.course_repo_name);
          ...
          try {
            const response = await axios.post(
              'http://localhost/api/account/student_excel_import',
              formData,
              {
                headers: {
          ...
          if (error.response && error.response.status === 400) {
            const missingStudents = error.response.data.missing_students;
            let alertMessage = 'Missing values for some students:\n';
      ...
      async fetchData() {
        ...
        this.loading = true;
          const response = await axios.get(
            'http://localhost/api/course/course_read_db'
        );
